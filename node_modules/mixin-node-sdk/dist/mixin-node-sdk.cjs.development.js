...
var forge__default = _interopDefault(forge);
...
var getSignPIN = function getSignPIN(keystore, pin, iterator) {
  ...
  var time = new int64Buffer.Uint64LE(Date.now() / 1000 | 0).toBuffer();

  if (iterator == undefined || iterator === '') {
    iterator = Date.now() * 1000000;
  }

  iterator = new int64Buffer.Uint64LE(iterator).toBuffer();
  ...
  var iv16 = crypto.randomBytes(16);
  var cipher = crypto.createCipheriv('aes-256-cbc', pinKey, iv16);
...
function getEd25519Sign(payload, privateKey) {
  ...
  var sign = base64url(Buffer.from(forge.pki.ed25519.sign({
    message: result.join('.'),
    encoding: 'utf8',
...
var signRequest = function signRequest(method, url, body) {
  ...
  return forge.md.sha256.create().update(method + url + body, 'utf8').digest().toHex();
};
...
function signPin(pin_token, private_key, session_id) {
  ...
  private_key = forge.pki.privateKeyFromPem(private_key);
...
var KeystoreAuth = /*#__PURE__*/function () {
  ...
  _proto.signToken = function signToken(signature, requestID) {
    ...
    if (!requestID) requestID = uuid.v4();
    var payload = {
      uid: client_id,
      sid: session_id,
      iat: issuedAt,
    ...
    return _privateKey.length === 64 ? getEd25519Sign(payload, _privateKey) : jsonwebtoken.sign(payload, private_key, {
      algorithm: 'RS512'
    });
  };
...
var newHash = function newHash(str) {
  return new sha3.SHA3(256).update(str).digest('hex');
};
var getFileByURL = function getFileByURL(url) {
  return new Promise(function (resolve, reject) {
...
var UserClient = /*#__PURE__*/function () {
      ...
      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              _forge$pki$ed25519$ge = forge__default.pki.ed25519.generateKeyPair(), publicKey = _forge$pki$ed25519$ge.publicKey, privateKey = _forge$pki$ed25519$ge.privateKey;
              params = {
                full_name: full_name,
  ...
  _proto.updateProfile = function updateProfile(params) {
...
var readAddresses = function readAddresses(token, assetID) {
  return request(undefined, token).get("/assets/" + assetID + "/addresses");
};
...
var AssetClient = /*#__PURE__*/function () {
  ...
  _proto.readAssetNetworkTicker = function readAssetNetworkTicker(asset, offset) {
...
function privateKeyToCurve25519$1(privateKey) {
  var h = crypto__default.createHash('sha512');
...
function generateUserCheckSum(_sessions) {
  var sessions = _sessions.map(function (v) {
    return v.session_id;
  });

  sessions.sort();
  var md5 = crypto__default.createHash('md5');
...
function decryptMessageData(_data, sessionID, _privateKey) {
  var data = Buffer.from(_data, 'base64');
  var privateKey = Buffer.from(_privateKey, 'base64');
  var size = 16 + 48;
  var total = data.length;
  if (total < 1 + 2 + 32 + size + 12) throw new Error('invalid data size');
  var sessionLen = data.readUInt16LE(1);
  var prefixSize = 35 + sessionLen * size;
  var key = Buffer.alloc(0);

  for (var i = 35; i < prefixSize; i += size) {
    var uid = uuid.v4({
      random: data.subarray(i, i + 16)
    });

    if (uid === sessionID) {
      var pub = data.subarray(3, 35);
      var priv = privateKeyToCurve25519$1(privateKey);
      var dst = tweetnacl.scalarMult(priv, pub);
      var iv = data.subarray(i + 16, i + 32);

      var _decipher = crypto__default.createDecipheriv('aes-256-cbc', dst, iv);

      key = data.subarray(i + 32, i + size);
      key = _decipher.update(key);
      key = Buffer.concat([key, _decipher["final"]()]);
      break;
    }
  }

  if (key.length !== 16) throw new Error('session id not found');
  var nonce = data.subarray(prefixSize, prefixSize + 12);
  var decipher = crypto__default.createDecipheriv('aes-128-gcm', key, nonce);
  decipher.setAuthTag(data.subarray(total - 16));
  var raw = decipher.update(data.subarray(prefixSize + 12, total - 16));
  raw = Buffer.concat([raw, decipher["final"]()]);
  return raw.toString('base64');
}
var encryptMessageData = function encryptMessageData(data, sessions, privateKey) {
  var key = crypto__default.randomBytes(16);
  var nonce = crypto__default.randomBytes(12);
  var cipher = crypto__default.createCipheriv('aes-128-gcm', key, nonce);
  var firstBlock = cipher.update(data);
  var lastBlock = cipher["final"]();
  var authTag = cipher.getAuthTag();
  var cipherText = Buffer.concat([firstBlock, lastBlock, authTag]);
  var sessionLen = Buffer.alloc(2);
  sessionLen.writeUInt16LE(sessions.length);
  var pub = ed25519.Point.fromHex(privateKey.subarray(32)).toX25519();
  var sessionsBytes = Buffer.from([]);

  for (var _iterator2 = _createForOfIteratorHelperLoose(sessions), _step2; !(_step2 = _iterator2()).done;) {
    var s = _step2.value;
    var clientPub = Buffer.from(s.public_key, 'base64');
    var priv = privateKeyToCurve25519$1(privateKey);
    var dst = tweetnacl.scalarMult(priv, clientPub);
    var iv = crypto__default.randomBytes(16);
    var padText = Buffer.alloc(16).fill(16);
    var shared = Buffer.concat([key, padText]);

    var _cipherText = Buffer.alloc(16 + shared.length);

    _cipherText.set(iv, 0);

    var _cipher = crypto__default.createCipheriv('aes-256-cbc', dst, iv);

    var enc = _cipher.update(shared);

    _cipherText.set(enc, 16);

    var id = uuid.parse(s.session_id);
    sessionsBytes = Buffer.concat([sessionsBytes, id, _cipherText]);
  }

  var result = Buffer.from([1]);
  result = Buffer.concat([result, sessionLen, pub, sessionsBytes, nonce, cipherText]);
  return result.toString('base64url');
};
var decryptAttachment = function decryptAttachment(data, _keys) {
  var aesKey = Buffer.from(_keys, 'base64').subarray(0, 32);
  var iv = data.subarray(0, 16);
  var cipherText = data.subarray(16, data.byteLength - 32);
  var decipher = crypto__default.createDecipheriv('aes-256-cbc', aesKey, iv);
...
var MessageClient = /*#__PURE__*/function () {
  ...
  _proto.sendEncryptMessage = function sendEncryptMessage(message) {
  ...
  _proto._sendEncryptMsg = /*#__PURE__*/function () {
    var _sendEncryptMsg2 = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee(userID, category, data, isRetry, message_id, conversation_id) {
      var sessions, data_base64, checksum, recipient_sessions, _yield$this$sendEncry, res;
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              return _context.abrupt("return", Promise.reject('category must start with ENCRYPTED_'));

            case 3:
              if (typeof data === 'object') data = JSON.stringify(data);
              _context.next = 6;
              return this.getSessionsWithCache([userID]);

            case 6:
              sessions = _context.sent;
              data_base64 = encryptMessageData(Buffer.from(data), sessions[userID], Buffer.from(this.keystore.private_key, 'base64'));
              checksum = generateUserCheckSum(sessions[userID] || []);
              recipient_sessions = sessions[userID].map(function (v) {
                return {
              ...
              _context.next = 14;
              return this.sendEncryptMessagesRaw([{
                category: category,
                recipient_id: userID,
            ...
            case 14:
              _yield$this$sendEncry = _context.sent;
              res = _yield$this$sendEncry[0];

              if (!(res.state === 'SUCCESS' || res.sessions.length === 0 || isRetry)) {
                _context.next = 18;
                break;
              }
            ...
            case 18:
              ...
              return _context.abrupt("return", this._sendEncryptMsg(userID, category, data, true, message_id));
    ...
    function _sendEncryptMsg(_x, _x2, _x3, _x4, _x5, _x6) {
      return _sendEncryptMsg2.apply(this, arguments);
    }

    return _sendEncryptMsg;
  }();

  _proto.sendEncryptMessagesRaw = function sendEncryptMessagesRaw(messages) {
    return this.request.post('/encrypted_messages', messages);
  };

  _proto.sendEncryptTextMsg = function sendEncryptTextMsg(userID, text) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_TEXT', text);
  };

  _proto.sendEncryptPostMsg = function sendEncryptPostMsg(userID, text) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_POST', text);
  };

  _proto.sendEncryptImageMsg = function sendEncryptImageMsg(userID, image) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_IMAGE', image);
  };

  _proto.sendEncryptDataMsg = function sendEncryptDataMsg(userID, data) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_DATA', data);
  };

  _proto.sendEncryptStickerMsg = function sendEncryptStickerMsg(userID, sticker) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_STICKER', sticker);
  };

  _proto.sendEncryptContactMsg = function sendEncryptContactMsg(userID, contact) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_CONTACT', contact);
  };

  _proto.sendEncryptAudioMsg = function sendEncryptAudioMsg(userID, audio) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_AUDIO', audio);
  };

  _proto.sendEncryptLiveMsg = function sendEncryptLiveMsg(userID, live) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_LIVE', live);
  };

  _proto.sendEncryptVideoMsg = function sendEncryptVideoMsg(userID, video) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_VIDEO', video);
  };

  _proto.sendEncryptLocationMsg = function sendEncryptLocationMsg(userID, location) {
    return this._sendEncryptMsg(userID, 'ENCRYPTED_LOCATION', location);
  };

  _proto.sendEncryptMessages = /*#__PURE__*/function () {
    var _sendEncryptMessages = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee2(_messages) {
      return runtime_1.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              return _context2.abrupt("return", this._sendEncryptMsgs(_messages));
    ...
    function sendEncryptMessages(_x7) {
      return _sendEncryptMessages.apply(this, arguments);
    }

    return sendEncryptMessages;
  }();

  _proto._sendEncryptMsgs = /*#__PURE__*/function () {
    var _sendEncryptMsgs2 = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee3(_messages, isRetry, resp) {
      var userIDs, sessions, msgs, msgMap, _iterator, _step, msg, category, recipient_id, data, message_id, conversation_id, session, data_base64, checksum, recipient_sessions, res, unsent, _iterator2, _step2, s;
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              ...
              userIDs = _messages.map(function (v) {
                return v.recipient_id;
              });
              _context3.next = 5;
              return this.getSessionsWithCache(userIDs);

            case 5:
              sessions = _context3.sent;
              msgs = [];
              msgMap = new Map();
              _iterator = _createForOfIteratorHelperLoose(_messages);
            ...
            case 9:
              ...
              category = msg.category, recipient_id = msg.recipient_id, data = msg.data, message_id = msg.message_id, conversation_id = msg.conversation_id;
              ...
              throw new Error('category must start with ENCRYPTED_');

            case 15:
              session = sessions[recipient_id];
              data_base64 = encryptMessageData(Buffer.from(data), session, Buffer.from(this.keystore.private_key, 'base64'));
              checksum = generateUserCheckSum(session || []);
              recipient_sessions = session.map(function (v) {
                return {
                  session_id: v.session_id
                };
              });
            ...
            case 24:
              _context3.next = 26;
              return this.sendEncryptMessagesRaw(msgs);

            case 26:
              res = _context3.sent;
              unsent = [];

              for (_iterator2 = _createForOfIteratorHelperLoose(res); !(_step2 = _iterator2()).done;) {
                s = _step2.value;
                if (s.state === 'SUCCESS' || isRetry) resp.push(s);else {
                  this.cacheSession(s.sessions);
                  unsent.push(msgMap.get(s.message_id));
                }
              }

              if (!(unsent.length === 0 || isRetry)) {
                _context3.next = 31;
                break;
              }
            ...
            case 31:
              return _context3.abrupt("return", this._sendEncryptMsgs(unsent, true, resp));
    ...
    function _sendEncryptMsgs(_x8, _x9, _x10) {
      return _sendEncryptMsgs2.apply(this, arguments);
    }

    return _sendEncryptMsgs;
  }();

  _proto.getSessions = /*#__PURE__*/function () {
    var _getSessions = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee4(userIDs) {
      return runtime_1.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            case 0:
              return _context4.abrupt("return", this.request.post('/sessions/fetch', userIDs));
  ...
  _proto.getSessionsWithCache = /*#__PURE__*/function () {
    var _getSessionsWithCache = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee5(userIDs) {
      var needFetch, result, _iterator3, _step3, userID, session, sessions, _result;
        ...
        while (1) {
          switch (_context5.prev = _context5.next) {
            case 0:
              needFetch = [];
              result = {};
              _iterator3 = _createForOfIteratorHelperLoose(userIDs);
            ...
            case 3:
              ...
              userID = _step3.value;
              session = this.getSessionByCache(userID);
            ...
            case 16:
              _context5.next = 18;
              return this.request.post('/sessions/fetch', needFetch);

            case 18:
              sessions = _context5.sent;
              _result = this.cacheSession(sessions);
              result = _extends({}, result, _result);
              return _context5.abrupt("return", result);
    ...
    function getSessionsWithCache(_x12) {
      return _getSessionsWithCache.apply(this, arguments);
    }

    return getSessionsWithCache;
  }();

  _proto.cacheSession = function cacheSession(sessions) {
    ...
    for (var userID in result) {
      ...
      var sessionPath = path.resolve(process.cwd(), '.sessions', userID + ".json");
      fs.writeFileSync(sessionPath, JSON.stringify(result[userID]));
    }

    return result;
  };

  _proto.decryptAttachmentByMsgData = /*#__PURE__*/function () {
    var _decryptAttachmentByMsgData = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee6(msgData) {
      var _yield$this$showAttac, view_url, raw;
        ...
        while (1) {
          switch (_context6.prev = _context6.next) {
            case 0:
              ...
              return _context6.abrupt("return", Promise.reject("msgData should be an object..."));
            ...
            case 2:
              ...
              return _context6.abrupt("return", Promise.reject("msgData should contain attachment_id..."));

            case 4:
              _context6.next = 6;
              return this.showAttachment(msgData.attachment_id);

            case 6:
              _yield$this$showAttac = _context6.sent;
              view_url = _yield$this$showAttac.view_url;
              _context6.next = 10;
              return getFileByURL(view_url);
            ...
            case 13:
              return _context6.abrupt("return", decryptAttachment(raw, msgData.key));
    ...
    function decryptAttachmentByMsgData(_x13) {
      return _decryptAttachmentByMsgData.apply(this, arguments);
    }

    return decryptAttachmentByMsgData;
  }();

  _proto.getSessionByCache = function getSessionByCache(userID) {
...
var Encoder = /*#__PURE__*/function () {
  ...
  _proto.writeUUID = function writeUUID(id) {
    var uuid$1 = uuid.parse(id);

    for (var i = 0; i < uuid$1.length; i++) {
      this.write(Buffer.from([uuid$1[i]]));
    }
  };
  ...
  _proto.encodeOutput = function encodeOutput(o) {
    ...
    this.writeInteger(new bignumber_js.BigNumber(1e8).times(new bignumber_js.BigNumber(o.amount)).toNumber());
...
function dumpTransaction(signed) {
  ...
  var e = Buffer.from(signed.extra, 'hex');
...
var MultisigsClient = /*#__PURE__*/function () {
  ...
  _proto.readMultisigOutput = function readMultisigOutput(id) {
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              change = inputs.reduce(function (sum, input) {
                return sum.plus(new bignumber_js.BigNumber(input.amount));
              }, new bignumber_js.BigNumber(0));

              for (_iterator2 = _createForOfIteratorHelperLoose(outputs); !(_step2 = _iterator2()).done;) {
                output = _step2.value;
                change = change.minus(new bignumber_js.BigNumber(output.amount));
              }

              if (change.gt(new bignumber_js.BigNumber(0))) outputs.push({
...
var TransferClient = /*#__PURE__*/function () {
  ...
  _proto.sendRawTransaction = function sendRawTransaction(raw) {
    return this.request.post('/external/proxy', {
...
var verifyPayment = function verifyPayment(params) {
  return mixinRequest.post('/payments', params);
};

var OauthClient = /*#__PURE__*/function () {
  function OauthClient() {}

  var _proto = OauthClient.prototype;
  ...
  _proto.getAuthorizeCode = /*#__PURE__*/function () {
    var _getAuthorizeCode = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee(params) {
      var client_id, _scopes, pin, _yield$this$getAuthor, authorization_id, scopes, pin_base64;

      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              client_id = params.client_id, _scopes = params.scopes, pin = params.pin;
              _context.next = 3;
              return this.getAuthorizeData(client_id, _scopes);

            case 3:
              _yield$this$getAuthor = _context.sent;
              authorization_id = _yield$this$getAuthor.authorization_id;
              scopes = _yield$this$getAuthor.scopes;
              pin_base64 = getSignPIN(this.keystore, pin);
              return _context.abrupt("return", this.request.post('/oauth/authorize', {
    ...
    function getAuthorizeCode(_x) {
      return _getAuthorizeCode.apply(this, arguments);
    }

    return getAuthorizeCode;
  }();

  _proto.getAuthorizeData = function getAuthorizeData(client_id, _scope) {
    var _this = this;

    return new Promise(function (resolve, reject) {
      var _scope2;

      var ws = new WebSocket('wss://blaze.mixin.one', 'Mixin-OAuth-1');
      if (!_scope) _scope = [];
      if (!_scope.includes('PROFILE:READ')) _scope.push('PROFILE:READ');
      var scope = (_scope2 = _scope) == null ? void 0 : _scope2.join(' ');

      var sendRefreshCode = function sendRefreshCode(authorization_id) {
        if (authorization_id === void 0) {
          authorization_id = '';
        }

        ws.send(pako.gzip(JSON.stringify({
          id: _this.newUUID().toUpperCase(),
      ...
      ws.addEventListener('message', function (event) {
        var msg = pako.ungzip(event.data, {
          to: 'string'
        });
...
function buildMintCollectibleMemo(collection_id, token_id, content) {
  ...
  encoder.writeSlice(Buffer.from(uuid.parse(collection_id)));
  encoder.writeSlice(Buffer.from(uuid.parse(token_id)));
...
var createCollectibleRequest = function createCollectibleRequest(token, action, raw) {
  return request(undefined, token).post("/collectibles/requests", {
    action: action,
    raw: raw
  });
};

var mvmRPCUri = 'https://geth.mvm.dev';
var registryAddress = '0x3c84B6C98FBeB813e05a7A7813F0442883450B1F';
var registryProcess = 'bd670872-76ce-3263-b933-3aa337e212a4';
var registryAbi = [{
  inputs: [{
    internalType: 'bytes',
    name: 'raw',
...
var OperationPurposeGroupEvent = 1; // const OperationPurposeAddProcess = 11
// const OperationPurposeCreditProcess = 12
// const mvmClient = axios.create({
//   baseURL: 'https://api.test.mvm.dev',
// });

var receivers = ['d5a3a450-5619-47af-a3b1-aad08e6e10dd', '9d4a18aa-9b0a-40ed-ba57-ce8fbbbc6deb', '2f82a56a-7fae-4bdd-bc4d-aad5005c5041', 'f7f33be1-399a-4d29-b50c-44e5f01cbb1b', '23a070df-6b87-4b66-bdd4-f009702770c9', '2385639c-eac1-4a38-a7f6-597b3f0f5b59', 'ab357ad7-8828-4173-b3bb-0600c518eab2'];
var threshold = 5;
var CNBAssetID = '965e5c6e-434c-3fa9-b780-c50f43cd955c';
var MinAmount = '0.00000001';
var getContractByAssetID = function getContractByAssetID(id) {
  return getRegistryContract().contracts('0x' + Buffer.from(uuid.parse(id)).toString('hex'));
};
var getContractByUserIDs = function getContractByUserIDs(ids, threshold) {
  if (typeof ids === 'string') ids = [ids];
  if (!threshold) threshold = ids.length;
  var encoder = new Encoder(Buffer.from([]));
  encoder.writeInt(ids.length);
  ids.forEach(function (id) {
    return encoder.writeUUID(id);
  });
  encoder.writeInt(threshold);
  return getRegistryContract().contracts(ethers.utils.keccak256('0x' + encoder.buf.toString('hex')));
};
var getAssetIDByAddress = /*#__PURE__*/function () {
  var _ref = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee(contract_address) {
    var registry, res;
    return runtime_1.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            registry = getRegistryContract();
            _context.next = 3;
            return registry.assets(contract_address);

          case 3:
            res = _context.sent;
            res instanceof ethers.BigNumber && (res = res._hex);
          ...
          case 7:
            res = res.slice(2);
            return _context.abrupt("return", uuid.stringify(Buffer.from(res, 'hex')));
  ...
  return function getAssetIDByAddress(_x) {
    return _ref.apply(this, arguments);
  };
}();
var getUserIDByAddress = /*#__PURE__*/function () {
  var _ref2 = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee2(contract_address) {
    var registry, res;
    return runtime_1.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            registry = getRegistryContract();
            _context2.next = 3;
            return registry.users(contract_address);
          ...
          case 7:
            res = res.slice(6);
            res = res.slice(0, 32);
            return _context2.abrupt("return", uuid.stringify(Buffer.from(res, 'hex')));
  ...
  return function getUserIDByAddress(_x2) {
    return _ref2.apply(this, arguments);
  };
}();

var getRegistryContract = function getRegistryContract(address) {
  if (address === void 0) {
    address = registryAddress;
  }

  return new ethers.ethers.Contract(address, registryAbi, new ethers.ethers.providers.JsonRpcProvider(mvmRPCUri));
};

var getMethodIdByAbi = function getMethodIdByAbi(methodName, types) {
  return ethers.utils.id(methodName + '(' + types.join(',') + ')').slice(2, 10);
};

var encodeMemo = function encodeMemo(extra, process) {
...
var getSingleExtra = function getSingleExtra(_ref3) {
  ...
  var inputLength = Buffer.from([0, contractInput.length / 2]).toString('hex');
  var extra = "" + addr + inputLength + contractInput;
  return extra;
};
/**  Get extra for multiple contracts calling, started with number of contracts to be called */


var getExtra = function getExtra(contracts) {
  if (contracts.length === 0) return '';
  var extra = Buffer.from([0, contracts.length]).toString('hex');

  for (var i = 0; i < contracts.length; i++) {
    var singleExtra = Buffer.from(getSingleExtra(contracts[i]));
    extra += singleExtra;
  }
...
var MvmClient = /*#__PURE__*/function () {
  function MvmClient() {}

  var _proto = MvmClient.prototype;

  _proto.paymentGeneratorByContract = /*#__PURE__*/function () {
    var _paymentGeneratorByContract = /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/runtime_1.mark(function _callee3(params) {
      var extra, _ref4, asset, amount, trace, _ref4$type, type, memo, txInput;

      return runtime_1.wrap(function _callee3$(_context3) {
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              if (!(!params.contract && !params.contracts)) {
                _context3.next = 2;
                break;
              }

              throw new Error('error: contract or contracts is required');

            case 2:
              if (params.contract) params.contracts = [params.contract];
              extra = getExtra(params.contracts);
              _ref4 = params.payment || {}, asset = _ref4.asset, amount = _ref4.amount, trace = _ref4.trace, _ref4$type = _ref4.type, type = _ref4$type === void 0 ? 'payment' : _ref4$type;
              memo = encodeMemo(extra, registryProcess).slice(2);
              txInput = {
              ...
              return _context3.abrupt("return", txInput);
            ...
            case 9:
              ...
              return _context3.abrupt("return", this.verifyPayment(txInput));
    ...
    function paymentGeneratorByContract(_x3) {
      return _paymentGeneratorByContract.apply(this, arguments);
    }

    return paymentGeneratorByContract;
  }();
...
[AddressClient, AppClient, AssetClient, AttachmentClient, ConversationClient, MessageClient, MultisigsClient, PINClient, SnapshotClient, TransferClient, UserClient, CollectiblesClient, MvmClient, OauthClient].forEach(function (client) {
  return _extends$1(Client, client);
});

function _extends$1(origin, target) {
  for (var key in target.prototype) {
    origin.prototype[key] = target.prototype[key];
  }
}

var authorizeToken = function authorizeToken(client_id, code, client_secret, code_verifier) {
  return mixinRequest.post('/oauth/token', {
    client_id: client_id,
    code: code,
    code_verifier: code_verifier,
    client_secret: client_secret
...
var BlazeClient = /*#__PURE__*/function (_Client) {
            ...
            switch (_context.prev = _context.next) {
              ...
              case 3:
                if (msg.category && msg.category.startsWith('ENCRYPTED_')) {
                  msg.data = decryptMessageData(msg.data_base64, _this2.keystore.session_id, _this2.keystore.private_key);
                }

                if ((_this2$options = _this2.options) != null && _this2$options.parse && msg.data) {
              ...
              case 22:
                ...
                _context.next = 25;
                return _this2.sendAcknowledgement({
  ...
  _proto.send_raw = function send_raw(message) {
    ...
    return new Promise(function (resolve) {
      ...
      var zipped = pako.gzip(buffer);

      if (_this4.ws.readyState === WebSocket.OPEN) {
        _this4.ws.send(zipped);
...
exports.BlazeClient = BlazeClient;
exports.Client = Client;
exports.MvmClient = MvmClient;
exports.authorizeToken = authorizeToken;
exports.createCollectibleRequest = createCollectibleRequest;
exports.decryptAttachment = decryptAttachment;
exports.getAssetIDByAddress = getAssetIDByAddress;
exports.getContractByAssetID = getContractByAssetID;
exports.getContractByUserIDs = getContractByUserIDs;
exports.getSignPIN = getSignPIN;
...
exports.readBlockUsers = readBlockUsers;
...
exports.sendExternalProxy = sendExternalProxy;
exports.userMe = userMe;
exports.verifyPayment = verifyPayment;
//# sourceMappingURL=mixin-node-sdk.cjs.development.js.map
