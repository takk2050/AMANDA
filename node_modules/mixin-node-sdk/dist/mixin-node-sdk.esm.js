import crypto__default, { randomBytes, createCipheriv, createHash } from 'crypto';
import { v4, parse, stringify } from 'uuid';
import axios from 'axios';
import { sign } from 'jsonwebtoken';
import forge, { util, pki, md } from 'node-forge';
import { Uint64LE } from 'int64-buffer';
import { SHA3 } from 'sha3';
import https from 'https';
import { existsSync, mkdirSync, writeFileSync } from 'fs';
import { resolve } from 'path';
import { scalarMult as scalarMult$1 } from 'tweetnacl';
import { Point } from '@noble/ed25519';
import { BigNumber } from 'bignumber.js';
import { ungzip, gzip } from 'pako';
import WebSocket from 'ws';
import { ethers, utils, BigNumber as BigNumber$1 } from 'ethers';
...
var encryptMessageData = function encryptMessageData(data, sessions, privateKey) {
  ...
  var pub = Point.fromHex(privateKey.subarray(32)).toX25519();
  ...
  for (var _iterator2 = _createForOfIteratorHelperLoose(sessions), _step2; !(_step2 = _iterator2()).done;) {
    ...
    var priv = privateKeyToCurve25519$1(privateKey);
    var dst = scalarMult$1(priv, clientPub);
...
var MessageClient = /*#__PURE__*/function () {
  ...
  _proto.cacheSession = function cacheSession(sessions) {
    ...
    for (var userID in result) {
      ...
      var sessionPath = resolve(process.cwd(), '.sessions', userID + ".json");
          ...
          switch (_context.prev = _context.next) {
            case 0:
              ...
              change = inputs.reduce(function (sum, input) {
                return sum.plus(new BigNumber(input.amount));
              }, new BigNumber(0));

              for (_iterator2 = _createForOfIteratorHelperLoose(outputs); !(_step2 = _iterator2()).done;) {
                output = _step2.value;
                change = change.minus(new BigNumber(output.amount));
              }

              if (change.gt(new BigNumber(0))) outputs.push({
...
var getContractByAssetID = function getContractByAssetID(id) {
  return getRegistryContract().contracts('0x' + Buffer.from(parse(id)).toString('hex'));
};
    ...
    return runtime_1.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            res = _context.sent;
            res instanceof BigNumber$1 && (res = res._hex);
          ...
          case 7:
            res = res.slice(2);
            return _context.abrupt("return", stringify(Buffer.from(res, 'hex')));
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 7:
            res = res.slice(6);
            res = res.slice(0, 32);
            return _context2.abrupt("return", stringify(Buffer.from(res, 'hex')));
...
var getRegistryContract = function getRegistryContract(address) {
  ...
  return new ethers.Contract(address, registryAbi, new ethers.providers.JsonRpcProvider(mvmRPCUri));
};

var getMethodIdByAbi = function getMethodIdByAbi(methodName, types) {
  return utils.id(methodName + '(' + types.join(',') + ')').slice(2, 10);
};
...
export { BlazeClient, Client, MvmClient, authorizeToken, createCollectibleRequest, decryptAttachment, getAssetIDByAddress, getContractByAssetID, getContractByUserIDs, getSignPIN, getUserIDByAddress, mixinRequest, readAddresses, readAsset, readAssets, readBlockUsers, readConversation, readExternalAddressesCheck, readExternalTransactions, readFriends, readNetworkAsset, readNetworkAssetsMultisig, readNetworkAssetsTop, readNetworkChains, readNetworkSnapshot, readNetworkSnapshots, readNetworkTicker, readSnapshot, readSnapshots, request, searchNetworkAsset, sendExternalProxy, userMe, verifyPayment };
